...
**Detailed-targeting workflow (search FIRST, then apply):** never invent a targeting id — Graph rejects made-up ids. 1) `meta_search_targeting` to find ids (validate names with `interest_validation` when the user supplied them); 2) pass them to `meta_create_adset` or `meta_update_targeting` in ONE of two representations:
- **Flat OR lists** — `interests`/`behaviors` (lists of `{id, name?}`): person matches ANY entry. The simple case.
- **`flexibleSpec` groups** — AND/OR combinations: entries WITHIN a group are OR'd, groups are AND'd together (`[{interests:[A,B]},{behaviors:[C]}]` = (A OR B) AND C). Max 10 groups (a conservative Klienta cap — Graph documents no hard limit); every group needs at least one entry.

Rules that hold for both: a passed list/object **REPLACES** the existing one, `[]` (or `{}` for exclusions) **clears** it — read the current targeting back to the user before replacing. Flat lists and `flexibleSpec` are **mutually exclusive in one call** (flat is Graph shorthand for a single group; combining is ambiguous and rejected). **Switching representation replaces the other:** setting a non-empty `flexibleSpec` removes any existing flat interests/behaviors from the ad set, and setting flat lists removes an existing `flexible_spec` — say so when proposing the change. `exclusions` (`{interests?, behaviors?}` — people matching ANY entry are excluded) may be combined with either representation and replaces the whole exclusions object. Quote the audience-size estimates when proposing targeting, labeled as estimates.
...
- **Targeting is the typed field set only:** geo (countries/regions/cities + excluded countries), age (13–65), gender, placements (four placement lists, or `automaticPlacements=true` for Advantage+ placements), the `advantageAudience` flag, and detailed targeting — flat interests/behaviors OR lists, `flexibleSpec` AND/OR groups, and `exclusions` (ids from `meta_search_targeting`, search first; see the detailed-targeting workflow above for replace/clear/XOR/representation-switch rules). NO custom audiences (they survive read-modify-write untouched but cannot be set here) — those still route to Ads Manager. There is no raw-JSON targeting passthrough.
